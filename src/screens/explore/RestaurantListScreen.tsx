...
import { useGetRestaurantsQuery } from '../../services/catalogApi';
    ...
    <ScreenContainer>
      ...
      <View style={{ marginTop: spacing.md }}>
        {isLoading ? (
          ...
          restaurants.map((r) => (
            <Pressable key={r.id} onPress={() => navigation.navigate('RestaurantDetail', { restaurantId: r.id })}>
              <Card style={styles.card}>
                <View style={styles.rowBetween}>
                  <Text style={styles.name}>{r.name}</Text>
                  <View style={styles.ratingRow}>
                    <Ionicons name="star" size={14} color={colors.secondary} />
                    <Text style={styles.rating}>{r.averageRating.toFixed(1)}</Text>
                  </View>
                </View>
                <Text style={styles.meta}>{r.city} · {r.priceRange}</Text>
                <Text style={styles.tags}>{r.cuisine.join(' · ')}</Text>
              </Card>
            </Pressable>
          ))
        ) : (
          <EmptyState title="No restaurants match" subtitle="Try a different city or budget filter." />
        )}
      </View>
    </ScreenContainer>
  );
