...
export default function WeatherLocationPicker({ visible, onClose, onSelect }: WeatherLocationPickerProps) {
  ...
  // One representative marker per city, for quick tap-to-select.
  ...
  // The dropped pin's own coordinates are what we check weather for — the nearest
  // known city is only used as a friendly label underneath.
  const nearestCityLabel = useMemo(() => {
    if (!pin || !pois || pois.length === 0) return 'Selected location';
    let best = pois[0];
    ...
    return `Near ${best.city}`;
  }, [pin, pois]);

  const confirmPin = () => {
    if (!pin) return;
    onSelect({ lat: pin.latitude, lng: pin.longitude, label: nearestCityLabel });
    setPin(null);
    onClose();
  };

  const confirmCity = (poi: { lat: number; lng: number; city: string }) => {
    onSelect({ lat: poi.lat, lng: poi.lng, label: poi.city });
    setPin(null);
    onClose();
  };
  ...
  return (
    <Modal visible={visible} animationType="slide" onRequestClose={onClose}>
      <View style={styles.container}>
        ...
        <Text style={styles.hint}>Tap a marker, or tap anywhere on the map to drop a pin and check that spot's weather.</Text>
        ...
        {pin ? (
          <Pressable style={styles.confirmBar} onPress={confirmPin}>
            <Ionicons name="partly-sunny" size={18} color={colors.textInverse} />
            <Text style={styles.confirmText}>Check weather here ({nearestCityLabel})</Text>
          </Pressable>
        ) : null}
      </View>
    </Modal>
